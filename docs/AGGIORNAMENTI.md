...
- **Esportazione Distinte Corriere**: Perfezionata la distinta corrieri. CORRIERE_B ed CORRIERE_C uniscono nella stessa riga i colli selezionati dello stesso corriere, giorno e indirizzo compatibile; i numeri conservano il formato compatto con `/` (es. `508213/34`) e colli, peso e importi si sommano. Corriere A genera una riga per ciascun collo con tutti i lotti uniti da ` + `; per Corriere A, colli e peso sono forzati a 1.
...
- **Spedizioni: parziali e distinte allineate**: il popover dati spedizione ora scrolla internamente, include autocomplete CAP/città e salva il singolo collo senza refresh pieno della vista. La distinta Corriere A conserva una riga per collo anche senza numero/lotto, mentre CORRIERE_B/CORRIERE_C restano compattate. Il COD/saldo logistico delle spedizioni parziali è calcolato sui soli prodotti spediti meno quota proporzionale dell'acconto ordine; la contabilità dell'ordine resta separata.
